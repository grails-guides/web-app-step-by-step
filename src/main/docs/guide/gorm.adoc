...
classes, we've defined a static `create` method which will create the `UserIceCream` instance for a given
`User` and `IceCream` instance.
<2> "Flush" tells GORM whether to immediately persist changes to the database, or whether
the change can wait till GORM decides to persist the current "session" (possibly along with
other database updates at the same time). We are using a default `flush` argument to turn flushing off by default,
